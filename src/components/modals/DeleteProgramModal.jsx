...
export default function DeleteProgramModal({ open, onClose, program, onSuccess }) {
    ...
    useEffect(() => {
        if (program) {
            setName(program.name.replace(/^BA /i, "Bachelor of Arts in ").replace(/^BS /i, "Bachelor of Science in "))
            setConfirmText("");
            setAgree(false);
        }
    }, [program])
    ...
    const isDisabled =
        confirmText !== program.name || !agree || loading;
        ...
        <div onClick={onClose} className="fixed inset-0 flex items-center justify-center bg-black/30 z-50">
            <div onClick={e => e.stopPropagation()} className="w-full max-w-md bg-white p-8 rounded-lg shadow-lg flex flex-col gap-6">
                ...
                <div className="flex justify-between items-center">
                    <h2 className="text-lg font-semibold text-red-600">Delete Program</h2>
                ...
                <p className="text-sm/5 text-gray-700">
                    You are about to permanently delete the program:
                ...
                <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium">Type <span className="font-semibold">"{program.name}"</span> to confirm:</label>
                    <input
                        type="text"
                ...
                <button
                    ...
                    `}
                >
                    {loading ? "Deleting..." : `Delete ${program.name}`}
                </button>
            </div>
        </div>
    );
