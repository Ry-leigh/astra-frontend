...
export default function DeleteClassTaskModal({ open, classCourseId, onClose, task, onSuccess }) {
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await api.delete(`/class/${classCourseId}/tasks/${task.id}`);
    ...
    const isDisabled =
        confirmText !== task.title || !agree || loading;
        ...
        <div onClick={onClose} className="fixed inset-0 flex items-center justify-center bg-black/30 z-50">
            <div onClick={e => e.stopPropagation()} className="w-full max-w-md bg-white p-8 rounded-lg shadow-lg flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium">Type <span className="font-semibold">"{task.title}"</span> to confirm:</label>
                    <input
                        type="text"
                ...
                <button
                    ...
                    `}
                >
                    {loading ? "Deleting..." : `Delete ${task.title}`}
                </button>
            </div>
        </div>
    );
